...
interface SearchSidebarProps {
  ...
  onReset: () => void;
  isLoading: boolean;
  onClose?: () => void; // Para fechar em mobile
}

const SearchSidebar: React.FC<SearchSidebarProps> = ({ onSearch, onFilesProcessed, onReset, isLoading, onClose }) => {
  ...
  return (
    <>
      ...
      <div className={`
        ...
        ${isMobile
          ? 'fixed inset-0 z-50 animate-slide-in'
          : 'relative'
        }
      `}>
        {/* Header Compacto */}
        <div className="p-3 md:p-4 border-b border-slate-700 flex-shrink-0 relative">
          {/* Botão X para mobile */}
          {isMobile && onClose && (
            <button
              onClick={onClose}
              className="absolute top-3 right-3 z-10 p-2 hover:bg-slate-800 rounded-full transition-colors"
